...
##Prgram Name : RF_finalProject
##First  Name : Chenxu
##Last   Name : Wang
##Student ID  : 10457625
##Purpose     : Use the Random Forest methodology to develop a classification model for the STATUS
...
fit<- randomForest(STATUS~.,data=training, importance=TRUE, na.action=na.omit, ntree=100)
importance(fit)
varImpPlot(fit)
...
#Check out the prediction of random forest model
table(actual=testing$STATUS, Prediction)

#Calculate the rate of wrong prediction
wrong<-(testing$STATUS!=Prediction)
error_rate<-sum(wrong)/length(testing$STATUS)
error_rate
